...
class EmailValidationView(View):
    def post(self, request):
        ...
        if User.objects.filter(email=email).exists():
            return JsonResponse({'email_error':'Sorry, email is in use. Please choose another one.'}, status=409)
...
class UsernameValidationView(View):
    def post(self, request):
        ...
        if User.objects.filter(username=username).exists():
            return JsonResponse({'username_error':'Sorry, username is in use. Please choose another one.'}, status=409)
    ...
    def post(self, request):
        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                ...
                email_body = 'Hi '+user.username+ 'Please use thid link to verify your account\n' +activate_url
                email = EmailMessage(
                    email_subject,
                    email_body,
...
class CompletePasswordReset(View):
    ...
    def post(self, request, uidb64, token):
        ...
        try:
            ...
            user.set_password(password)
            user.save()
            messages.success(request, 'Password changed successfully. Please log in using your new password. ')
            return redirect('login')
        except Exception as identifier:
            messages.info(request, 'Something went wrong, try again.')
            return render(request, 'authentication/set-new-password.html', context)
